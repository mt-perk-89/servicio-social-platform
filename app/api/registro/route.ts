...
// Centro de referencia (CDMX) por si el alumno no comparte ubicación exacta.
...
export async function POST(request: Request) {
  try {
    ...
    const token = await createToken({ correo, nombre, rol: "alumno" })

    return NextResponse.json({
      token,
      alumno: { correo, nombre, rol: "alumno" },
    })
  } catch (error) {
